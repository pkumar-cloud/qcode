...
function check_service_status(){
  ...
  then
    print_color "green" "$1 service is active and running"
  else
    print_color "red" "$1 is service not active/running"
    exit 1
  fi
}
...
do
  check_item "$web_page" $item #Any argument has space, encapsulate in ""
done
